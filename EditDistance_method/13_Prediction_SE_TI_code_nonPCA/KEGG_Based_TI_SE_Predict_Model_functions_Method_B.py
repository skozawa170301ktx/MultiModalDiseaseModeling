...
Methods :
1. When a target is presented in the form of a KEGG ID, the pathway map associated with it is retrieved.
2. Build that pathway map as a network and search for relevant Paths.
...
def build_network(KEGG_ID, kegg_pathwayid, df_node, df_edge):
    ...
    Args:
        kegg_pathwayid (str): Targeting pathway map IDs
...
def PATHs_Search(KEGG_ID):
    """ Programs to search for Paths related to component
    Args:
        KEGG_ID (int): KEGG ID of component
        
    Returns:
        PATH_list (list): List of Paths associated with component
...
def Clustering_TI(Levenshtein_matrix, TI_ID):
    """ Program to sample the features for each TI (Threshhold can be changed)
    Args:
        Levenshtein_matrix (csr_matrix): A matrix that the value is calclated by (1 - Levenshtein ratio)
        
    Returns:
        Clustring_matrix (csr_matrix): A matrix converted by sampling

    """
    # 4. Input the matrix into each SE model and produce the result. (Threshhold should be modifiable.)
    
    y = pd.read_csv('../4_Feature_extraction_nonPCA/output/TI_Feature_Cluster_ID.csv',header = 0, index_col=0)

    Clustering_matrix = Levenshtein_matrix[:, list(y[y['TI_ID']==TI_ID]['ID'])]

    return Clustering_matrix


def LGBM_TI(Clustering_matrix, TI_ID, TI_Name ,Threshhold):
    """ Program that makes predictions using a matrix converted by sampling
    Args:
        Clustring_matrix (csr_matrix): A matrix converted by sampling
    ...
    try:
        ...
        for k in range(10):
            ...
            m = pd.read_pickle('../11_LGBM_TI_nonPCA/model_ti_' + str(TI_ID) + '_' + str(k) + '.pkl')
            y_pred = m.predict(Clustering_matrix, num_iteration = m.best_iteration)
...
def TI_Prediction_From_TARGET(df_KEGG_ID, Threshhold = 0.5, TI_list = None):
    """ A main program that makes TI predictions using Paths starting from component as input values
    Args:
    ...
    for KEGG_ID in df_KEGG_ID['KEGG_ID']:
        try:
            ...
            for TI_ID, TI_Name in zip(df_use['ID'], df_use['TI_name']):
                ...
                Clustering_matrix = Clustering_TI(Levenshtein_matrix, TI_ID)
                df_TI = LGBM_TI(Clustering_matrix, TI_ID, TI_Name, Threshhold).rename(columns = {'TI':KEGG_ID})
...
def Clustering_SE(Levenshtein_matrix, SE_ID):
    """ Program to sample the features for each SE (Threshhold can be changed)
    Args:
        Levenshtein_matrix (csr_matrix): A matrix that the value is calclated by (1 - Levenshtein ratio)
    ...
    y = pd.read_csv('../4_Feature_extraction_nonPCA/output/SE_Feature_Cluster_ID.csv',header = 0, index_col=0)

    Clustering_matrix = Levenshtein_matrix[:, list(y[y['SE_ID']==SE_ID]['ID'])]

    return Clustering_matrix


def LGBM_SE(Clustering_matrix, SE_ID, SE_Name ,Threshhold):
    """ Program that makes predictions using a matrix converted by sampling
    Args:
        Clustring_matrix (csr_matrix): A matrix converted by sampling
        Threshhold (float): Threshold for prediction
    ...
    try:
        ...
        for k in range(10):
            ...
            m = pd.read_pickle('../11_LGBM_SE_nonPCA/model_se_' + str(SE_ID) + '_' + str(k) + '.pkl')
            y_pred = m.predict(Clustering_matrix, num_iteration = m.best_iteration)
            pred = list(np.where(y_pred < Threshhold, 0, 1))
            y_preds.append(pred)
        y_pred_new = np.where(np.average(y_preds, axis = 0) <= 0.5, 0, 1)
...
def SE_Prediction_From_TARGET(df_KEGG_ID, Threshhold = 0.5, SE_list = None):
    """ A main program that makes SE predictions using Paths starting from component as input values
    Args:
        df_KEGG_ID (DataFrame): Dataframe of KEGG ID
        Threshhold (float): Threshold for prediction
    ...
    for KEGG_ID in df_KEGG_ID['KEGG_ID']:
        try:
            ...
            for SE_ID, SE_Name in zip(df_use['ID'], df_use['SE_name']):
                Clustering_matrix = Clustering_SE(Levenshtein_matrix, SE_ID)
                df_SE = LGBM_SE(Clustering_matrix, SE_ID, SE_Name, Threshhold).rename(columns = {'SE':KEGG_ID})
...
def SE_TI_Prediction_From_TARGET(KEGG_ID_i, Threshhold = 0.5, max_PATH = 2500, SE_list = None, TI_list = None):
    """ A main program that makes SE/TI predictions using Paths starting from component as input values
    Args:
        df_KEGG_i (int): KEGG ID
        Threshhold (float): Threshold for prediction
    ...
    try:
        ...
        if len(PATH_list) > max_PATH:
            ...
            for SE_ID, SE_Name in zip(df_use['ID'], df_use['SE_name']):
                Clustering_matrix = Clustering_SE(Levenshtein_matrix, SE_ID)
                df_SE = LGBM_SE(Clustering_matrix, SE_ID, SE_Name, Threshhold).rename(columns = {'SE':KEGG_ID_i})
                df_Target = pd.concat([df_Target, df_SE])
            df_all_Target = pd.merge(df_all_Target, df_Target, left_on = 'SE_name', right_index=True)
            ...
            for TI_ID, TI_Name in zip(df_use['ID'], df_use['TI_name']):
                ...
                Clustering_matrix = Clustering_TI(Levenshtein_matrix, TI_ID)
                df_TI = LGBM_TI(Clustering_matrix, TI_ID, TI_Name, Threshhold).rename(columns = {'TI':KEGG_ID_i})
                df_Target = pd.concat([df_Target, df_TI])
            df_all_Target = pd.merge(df_all_Target, df_Target, left_on= 'TI_name', right_index=True)
